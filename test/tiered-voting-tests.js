let RegistrationPool = artifacts.require("./public-elections/RegistrationPool.sol");
let Election = artifacts.require("./public-elections/Election.sol");
...
contract('Base Election', (accounts)=> {
    ...
    // create elections, and cast two district-level votes
    beforeEach(() => {
        return Election.new(ref, {from: admin}).then((e) => {
            federal = e;
            return Election.new(ref, {from: admin})
        }).then((e) => {
            state = e;
            return Election.new(ref, {from: admin})
        }).then((e) => {
            ...
            district2 = e;
            return RegistrationPool.new([district1.address, state.address, federal.address], gasAmt, [web3.sha3(pin)], {from: admin});
        }).then((e) => {
            localElection1 = e;
            return RegistrationPool.new([district2.address, state.address, federal.address], gasAmt, [web3.sha3(pin)], {from: admin});
        }).then ((e) => {
            localElection2 = e;
            ...
            return localElection1.castVotes(vote1, {from: voter1})
        }).then(() => {
            return localElection2.castVotes(vote2, {from: voter2})
        })
    });
